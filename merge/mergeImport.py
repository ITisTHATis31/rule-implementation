...
class RBCM_C_RULE_016():
    ...
    def main_script(cls, file_path):
        ...
        header_files = find_files.find_header_files(file_path)
        ...
        if header_files:  # If the path of the parent folder is given
            ...
            for header_file in header_files:
                ...
                lines_with_comments_removed = multiline.remove_comments(header_file)  # remove_comments returns a list
            ...
            if file_path.endswith(".h"):
                ...
                lines_with_comments_removed = multiline.remove_comments(file_path)  # remove_comments returns a list
                found_code = RBCM_C_RULE_016.check_executable_code_in_header(lines_with_comments_removed)
                if found_code:
                    logger.info("The header file contains executable code.")
                else:
...
class RBCM_C_RULE_035:
    ...
    def main_script(cls, file_path):
        c_files = find_files.find_c_files(file_path)
...
class RBCM_C_RULE_037:
    ...
    def check_blocks(file_path):
        try:
            lines = multiline.remove_comments(file_path)
...
class RBCM_C_RULE_010:
    ...
    def check_function(file_path):
        ...
        c_lines_without_comments = multiline.remove_comments(file_path)
...
class RBCM_C_RULE_011:
    ...
    def main_script(cls, file_path):
        h_files = find_files.find_header_files(file_path)
